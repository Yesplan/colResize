...
/* Yesplan adaptation:
    adapted this library to handle multiple header rows.
    To make this work the following needs to be ensured:
       - the first row in the header of the table must be a row with as much cells (<th>s) as there are columns.
         These cells can be styled to be hidden and need no content, but the "data-column-index" property needs to be manually set (starting from 0).
         This is because the datatables library only adds this property for header cells that are actual column's.
       - header cells that have 'subheader' cells must have a data-columnname attribute which is unique.
       - header cells that are grouped by a topheader cell must refer to it by the data-parentcolumnname attribute, which is the same as the above.
 */
(function (window, document, undefined) {
  ...
  var factory = function ($, DataTable) {
    ...
    ColResize.prototype = {
      ...
      /* Auxiliary function that returns the index of the column in the datatable.
         Takes into account that group headers have no data attribute for the index
       */
      _getColumnIndexWhileResizing: function(dom, dt) {
        var idx = parseInt(dom.resizeCol.attr("data-column-index"));
        var idxNeighbour;

        // the last column has no 'right-side' neighbour
        // with fixed this can make the table smaller
        if (dom.resizeColNeighbour[0] === undefined) {
          if (isNaN(idx)) {
            idx = dt.aoColumns.length - 1;
          }
          idxNeighbour = 0;
        } else {
          idxNeighbour = parseInt(dom.resizeColNeighbour.attr("data-column-index"));
        }
        ...
        /* Only the current column covers multiple columns */
        if (isNaN(idx) && idxNeighbour > 0) {
          idx = idxNeighbour - 1;
        }
        return {
          index: idx,
          indexNeighbour: idxNeighbour
        };
      },
      ...
      "_fnMouseDown": function (e, nTh) {
        ...
        var idxs = this._getColumnIndexWhileResizing(that.dom, that.s.dt);
        var idx = idxs.index;
        ...
        if (that.dom.resizeColNeighbour[0] === undefined) {
          ...
          if (isNaN(idxNeighbour))
            idxNeighbour = idxs.indexNeighbour;
        }
      ...
      "_fnMouseMove": function (e) {
        ...
        if ((dx > 0 && distFromRight <= 0) || (dx < 0 && distFromRight >= 0)) {
          if (!that.s.init.tableWidthFixed) {
            ...
            var colResizeIdx = that._getColumnIndexWhileResizing(that.dom, that.s.dt).index;
            ...
            if (that.s.mouse.targetColumn.parentcolumnname) {
              var columnParent = $('th').filter(function() { return $(this).attr("data-columnname") === that.s.mouse.targetColumn.parentcolumnname; });
      ...
      "_fnResizeHandleCheck": function (e, nTh) {
        ...
        if (that.dom.resizeCol && $(that.dom.resizeCol).attr("data-column-index")) {
          index = parseInt($(that.dom.resizeCol).attr("data-column-index"));
        }

        //If table width is fixed make sure both columns are resizable else just check the one.
        if (this.s.init.tableWidthFixed)
          resizeAvailable &= this.s.init.exclude.indexOf(index) == -1 && this.s.init.exclude.indexOf(parseInt($(that.dom.resizeColNeighbour).attr("data-column-index"))) == -1;
        else if (index !== undefined)
          resizeAvailable &= this.s.init.exclude.indexOf(index) == -1;
      ...
      "_fnMouseUp": function (e) {
        ...
        if (that.s.init.resizeCallback) {
          /* Assign the width: in the case of a drag on a parent level of a nested header row,
             we take the width of the rightmost child in this case instead of the total width of the parent level.
           */
          that.s.mouse.targetColumn.width = $(that.s.mouse.targetColumn.nTh).width();
